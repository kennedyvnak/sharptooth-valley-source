...
namespace NFHGame.LevelAssets.Level5 {
    public class NoiseLineDrawer : MonoBehaviour {
        private readonly static int[] s_Perm = {
        ...
        [SerializeField] private RangedFloat m_Range;
        ...
        public int wave { get => m_Wave; set => m_Wave = value; }
        public int points { get => m_Points; set => m_Points = value; }
#if UNITY_EDITOR
        public new LineRenderer renderer => m_Renderer;
#else
        public LineRenderer renderer => m_Renderer;
#endif
        ...
        private void Draw() {
            m_Renderer.positionCount = m_Points;
            for (int i = 0; i < m_Points; i++) {
                float progress = (float)i / (m_Points - 1);
                float x = Mathf.Lerp(m_Range.min, m_Range.max, progress);
                float y = wave switch {
                    2 => m_Amplitude * Noise(x * m_Frequency + Mathf.Cos(Time.time * m_Speed)),
                    1 => m_Amplitude * Noise(x * m_Frequency + Mathf.Sin(Time.time * m_Speed)),
                    _ => m_Amplitude * Noise(x * m_Frequency + Time.time * m_Speed),
                };
                m_Renderer.SetPosition(i, new Vector3(x, y, 0.0f));
            }
        }
        ...
        public static float Noise(float x, float y) {
            ...
            return Lerp(v, Lerp(u, Grad(s_Perm[A], x, y), Grad(s_Perm[B], x - 1, y)),
        ...
        public static float Noise(float x, float y, float z) {
            ...
            return Lerp(w, Lerp(v, Lerp(u, Grad(s_Perm[AA], x, y, z), Grad(s_Perm[BA], x - 1, y, z)),
