...
namespace NFHGame.Serialization.Handlers {
    public class FileDataHandler : DataHandler {
        ...
        public override GameData Deserialize(int userId) {
            ...
            if (File.Exists(path)) {
                try {
                    ...
                    GameLogger.data.LogError($"Caught an error trying to deserialize data file '{path}'.\n{e}");
                }
                data.userId = userId;
            }
            return data;
        ...
        public override GlobalGameData DeserializeGlobal() {
            GlobalGameData data = null;
            ...
            if (File.Exists(path)) {
                try {
                    using FileStream stream = new FileStream(path, FileMode.Open);
                    using StreamReader reader = new StreamReader(stream);
                    data = JsonUtility.FromJson<GlobalGameData>(EncryptDecrypt(reader.ReadToEnd()));
                } catch (Exception e) {
                    GameLogger.data.LogError($"Caught an error trying to deserialize data file '{path}'.\n{e}");
        ...
        public override void Serialize(GameData data, int slot) {
            ...
            try {
                ...
                GameLogger.data.LogError($"An error was detected when trying to serialize data to a file in '{path}'.\n{e}");
            }
        }
